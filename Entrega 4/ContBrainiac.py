...
class op_bin:
    ...
    def evaluate(self):
        if self.op == 'Suma':
            ...
            if self.left.type == 'tape':
                    r = [min(left[0],right[0]),max(left[1],right[1])]
            else:
                    r = left + right
        elif self.op == 'Resta':
            left = self.left.evaluate()
            ...
            right = self.right.evaluate()                                
            if right == 0:
                return "Error: División módulo entre cero."
            else:
                r = left % right
            ...
            if right == 0:
                ...
                r = left / right
        elif self.op == 'Desigual':
            left = self.left.evaluate()
            right = self.right.evaluate()                                
            if self.left.type == 'tape':
                r = left[0] != right[0] or left[1] != right[1]
            else:
                r = left != right
...
class inst_for:
    ...
    def checktype(self):
        ...
        while clone:
            ...
            break
    def execute(self):
        r = self.rango.evaluate()
        x = r[0] 
        st = SymTable.symtable()
        st.insert(self.var.name,['integer',x])
        index = len(stack) 
        stack.append(st.cloneSymtable())
        for x in range(r[0],r[1]+1):
            self.inst.execute()
            stack[index].update(self.var.name,['integer',x+1])
        stack.pop()
...
class inst_read:
    ...
    def execute(self):
        ...
        valUser = 0
        while i >= 0:
            if stack[i].isMember(self.var):
                typevar = stack[i].findType(self.var)
                validate = False
                while not validate:
                    strUser = raw_input()
                    if typevar == 'integer':
                        validate = funciones.is_int(strUser)    
                        if validate:
                            valUser = int(strUser)
                            validate = funciones.num_valid(valUser)
                            if not validate:
                                print "Error: Ingresaste un numero mayor de 32 bits"
                        elif typevar == 'tape':
                            p = False
                            if '..' in strUser:
                                strVal = strUser.split('..')    
                                if len(strVal) == 2:
                                    p = True
                            elif ',' in strUser:
                                strVal = strUser.split(',') 
                                if len(strVal) == 2:
                                    p = True
                            if p:
                                validate = funciones.is_int(strVal[0]) 
                                validate = validate and (funciones.is_int(strVal[1]))
                                if validate:
                                    validate = validate and funciones.num_valid(int(strVal[0]))
                                    validate = validate and funciones.num_valid(int(strVal[1]))
                                    validate = validate and (int(strVal[0]) <= int(strVal[1]))
                                    
                            if validate:
                                valUser = [int(strVal[0]),int(strVal[1])]
                        elif typevar == 'boolean':
                            if strUser == 'true':
                                valUser = True
                                                                validate = True
                            elif strUser == 'false':
                ...
                stack[i].addValueToKey(self.var,valUser)
                break
        i -= 1
...
class bloque:
    ...
    def execute(self):
        if self.declare != None:
            stack.append(self.declare.symtable.cloneSymtable())
        self.l.execute()
        if self.declare != None:
                stack.pop()
